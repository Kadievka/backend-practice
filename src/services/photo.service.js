...
export default class PhotoService {
  ...
  static async getOnePhotoByAuthorService(name, userId) {
    ...
    logger.debug(
      `[getOnePhotoByAuthorService] photoName: ${photoName}, type: ${type}, userId: ${userId}`
    );
    logger.debug(`[getOnePhotoByAuthorService] FINISH`);
    return this.getOnePhotoByNameTypeAndAuthor(photoName, type, userId, true);
  }

  static async getOnePhotoByNameTypeAndAuthor(
    name,
    type,
    userId,
    throwErrorIfNoExists = false
  ) {
    let photo = null;
  ...
  static getPhotosByAuthorService(userId, optionsRequest) {
    ...
    logger.debug(
      `[getPhotosByAuthorService] optionsRequest: ${JSON.stringify(
        optionsRequest
      )}`
    );
    const query = {
  ...
  static async getPhotosFromAPIService(optionsRequest) {
    ...
    logger.debug(
      `[getPhotosFromAPIService] optionsRequest: ${JSON.stringify(
        optionsRequest
      )}`
    );
    return fetch(`${process.env.JPH_API}/photos`)
      ...
      .then((photos) => {
        return paginateUtil(
          photos || [],
          optionsRequest.limit || defaultConstants.PHOTOS_LIMIT_NUMBER,
          optionsRequest.page || 1
        );
      });
  }
}
